...
const NGN_TO_USD = 1 / 1650; // approximate fixed rate; informational display only
...
export default function EnergyTracker() {
  ...
  const [currency, setCurrency] = useState("NGN"); // "NGN" | "USD" — display only, doesn't affect stored data
  ...
  useEffect(() => {
    try {
      ...
      const savedCurrency = localStorage.getItem("energy-tracker-currency");
  ...
  useEffect(() => {
    if (!loaded) return;
    try {
      localStorage.setItem("energy-tracker-currency", currency);
    } catch (e) {
      console.error("Save failed", e);
    }
  }, [currency, loaded]);
  ...
  return (
    <div className="min-h-screen bg-[#0D1117] text-[#E6E9ED] font-sans pb-16">
      ...
      <header className="px-5 pt-8 pb-6 max-w-3xl mx-auto">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center bg-[#1C2530] border border-[#2A3441] rounded-full p-0.5">
              <button
                onClick={() => setCurrency("NGN")}
                className={`font-mono text-[11px] px-2.5 py-1 rounded-full transition-colors ${
                  currency === "NGN"
                    ? "bg-[#F2C94C] text-[#0D1117] font-semibold"
                    : "text-[#8B95A1]"
                }`}
              >
                ₦ NGN
      ...
      {currency === "USD" && (
        <div className="px-5 max-w-3xl mx-auto -mt-4 mb-4">
          <p className="font-mono text-[10px] text-[#8B95A1] text-right">
            Approximate conversion · ₦1,650 ≈ $1
          </p>
        </div>
      )}
      ...
      <main className="px-5 max-w-3xl mx-auto space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <StatCard
            icon={<Wallet size={16} />}
            label="Avg cost / day"
            value={fmtMoney(stats.avgCostPerDay, currency)}
            sub="this month"
          />
          <StatCard
            icon={<TrendingUp size={16} />}
            label="Month spend"
            value={fmtMoney(stats.totalMonthlySpend, currency)}
...
function RecentActivity({ data, currency }) {
  const items = useMemo(() => {
    const all = [
      ...
      ...data.genLogs.map((g) => ({
        ts: g.date + " 12:00",
        text: `Generator: ${fmtHrs(g.hours)}, ${g.fuelLiters || 0}L for ${fmtMoney(g.fuelCost, currency)}`,
        color: "#3FB68B",
      })),
      ...data.meterLogs.map((m) => ({
        ts: m.date + " 12:00",
        text: `Meter token: ${m.units || 0} kWh for ${fmtMoney(m.cost, currency)}`,
        color: "#5B8DEF",
      })),
    ];
